...
if (overviewBody.includes('setAttribute("tabindex", "0")') || overviewBody.includes("setAttribute('tabindex', '0')")) throw new Error('Player overview must not use tabindex=0');
if (!overviewBody.includes('summary.removeAttribute("aria-label")')) throw new Error('Player overview must not depend on aria-label');
...
console.log(`Stable accessibility wiring OK: ${helpers.length} helpers, single real-text header overview contract present.`);
